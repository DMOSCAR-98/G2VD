"""
DeMamba-CLIP video backbone (project adaptation).

Follows the CLIP_DeMamba path in DeMamba.py:
...
typical from_pretrained usage for CLIPVisionModel).
...
class DeMambaCLIPVideoBackbone(nn.Module):
    """
    Video backbone based on DeMamba-CLIP model.

    DeMamba-CLIP model characteristics:
    - Uses CLIPVisionModel as vision_encoder and Mamba blocks for temporal modeling
    ...
    - Also returns frame_level_reps: per-frame vectors from vision_encoder pooler_output
      (optional downstream use; aligns with HF CLIPVisionModel per-frame layout).
    - Difference from DeMamba.py CLIP global feature definition: this adaptation uses
      mean over per-frame pooler_output (CLS-like frame descriptors), while upstream CLIP
      code averages across all frame tokens (CLS + patches).
    ...
    - Project adaptation vs DeMamba.py CLIP branch: this module applies branch-wise
      normalization to global and mamba features before concatenation and final
      projection.
    - Patch geometry (patch_grid, patch_count, fused_patch_count) is derived from
      vision_cfg image_size and patch_size at init. fusing_ratios is fixed at 2 for
      the DeMamba CLIP branch and must match the default hf_repo
      (clip-vit-base-patch16). For ViT patch16 @224: patch_count=196, fused_patch_count=49,
    ...
    def __init__(
        ...
        """
        Initialize the DeMamba-CLIP video backbone.
        ...
        # DeMamba CLIP branch: fusing_ratios=2 (see class docstring).
    ...
    def forward(
        ...
        """
        Forward pass through CLIP vision encoder and DeMamba temporal pooling.
