...
from ..modules.ae_modules import Normalize, nonlinearity
from ..modules.attention_temporal_videoae import *
...
class AutoencoderKL2plus1D_1dcnn_Standard(nn.Module):
    """
    Standard PyTorch Module version of AutoencoderKL2plus1D_1dcnn.
    Removed Lightning-specific functionality for more flexible usage.
    """
    def __init__(
        self,
        ddconfig: Dict[str, Any],
        ppconfig: Dict[str, Any],
        embed_dim: int = 0,
        use_quant_conv: bool = True,
        caption_guide: bool = False,
        t5_model_max_length: int = 120,
    ) -> None:
        """
        Initialize the standard autoencoder.

        Args:
            ddconfig (Dict[str, Any]): Configuration dictionary for 2+1D encoder/decoder.
            ppconfig (Dict[str, Any]): Configuration dictionary for temporal encoder/decoder.
            embed_dim (int): Embedding dimension for quantization (default: 0).
            use_quant_conv (bool): Whether to use quantization convolutions.
            caption_guide (bool): Whether to use caption guidance (for external text encoding).
            t5_model_max_length (int): Maximum length for T5 text encoder (for external use).
        """
        super().__init__()
        self.use_quant_conv = use_quant_conv
        self.caption_guide = caption_guide
        ...
        if use_quant_conv:
            assert embed_dim, "embed_dim must be positive when use_quant_conv=True"
            self.embed_dim = embed_dim
    ...
    def encode_temporal(
        ...
        Args:
            ...
            text_embeddings (Optional[torch.Tensor]): Optional text embeddings.
            text_attn_mask (Optional[torch.Tensor]): Optional attention mask for text.

        Returns:
            DiagonalGaussianDistribution: Posterior distribution.
        """
        moments = self.encoder_temporal(x, text_embeddings, text_attn_mask)
        posterior = DiagonalGaussianDistribution(moments)
        return posterior
    ...
    def encode_2plus1d(
        ...
        text_embeddings: Optional[torch.Tensor] = None,
        text_attn_mask: Optional[torch.Tensor] = None,
        mask_temporal: bool = False,
    ) -> DiagonalGaussianDistribution:
        """
        Encode spatial-temporal features using 2+1D encoder.

        Args:
            x (torch.Tensor): Input tensor of shape [B, C, T, H, W].
            text_embeddings (Optional[torch.Tensor]): Optional text embeddings.
            text_attn_mask (Optional[torch.Tensor]): Optional attention mask for text.
            mask_temporal (bool): Whether to mask temporal operations.

        Returns:
            DiagonalGaussianDistribution: Posterior distribution.
        """
        h = self.encoder(x, text_embeddings, text_attn_mask, mask_temporal)
        if self.use_quant_conv:
            h = self.quant_conv(h)
        posterior = DiagonalGaussianDistribution(h)
        return posterior
    ...
    def decode_2plus1d(
        ...
        """
        Decode spatial-temporal features using 2+1D decoder.
        ...
        if self.use_quant_conv:
            z = self.post_quant_conv(z)
        dec = self.decoder(z, text_embeddings, text_attn_mask, mask_temporal)
        return dec
    ...
    def encode(
        ...
        """
        Encode input through 2+1D and temporal encoders.
        ...
        Returns:
            Tuple[torch.Tensor, DiagonalGaussianDistribution]: Tuple of (latent tensor,
                posterior distribution).
        """
        posterior = self.encode_2plus1d(
            x, text_embeddings, text_attn_mask, mask_temporal
        )
        if sample_posterior:
            z = posterior.sample()
        else:
            z = posterior.mode()
        # Ensure z is on the same device and dtype as input x
    ...
    def decode(
        ...
        """
        Decode latent through temporal and 2+1D decoders.
        ...
        if not mask_temporal:
            z = self.decode_temporal(z, text_embeddings, text_attn_mask)
        dec = self.decode_2plus1d(
            z, text_embeddings, text_attn_mask, mask_temporal
        )
        return dec
    ...
    def forward(
        ...
        Args:
            inputs (torch.Tensor): Input tensor of shape [B, C, T, H, W].
            text_embeddings (Optional[torch.Tensor]): Optional text embeddings.
            text_attn_mask (Optional[torch.Tensor]): Optional attention mask for text.
            sample_posterior (bool): Whether to sample from posterior or use mode.
            mask_temporal (bool): Whether to mask temporal operations.
        ...
        Returns:
            Tuple[torch.Tensor, DiagonalGaussianDistribution]: Tuple of (reconstructed tensor,
                posterior distribution).
        """
        z, posterior = self.encode(
            inputs,
