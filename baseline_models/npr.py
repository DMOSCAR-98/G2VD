"""
NPR baseline for generalizable deepfake detection; use BaselineModelWrapper with BTCHW input.
...
def resnet50_npr(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet.
            Default False to match original NPR source; model uses 3x3 stem,
            which is incompatible with the official 7x7 ResNet50 checkpoint.
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        # Pretrained weights (legacy .tar); trusted source — use weights_only=False for PyTorch 2.6+.
        model_state = torch.load(
            './pretrained_weights/resnet50-19c8e357.pth',
            map_location='cpu',
            weights_only=False,
        )
