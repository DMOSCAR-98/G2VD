"""
TALL SWIN baseline components; consumed via BaselineModelWrapper with BTCHW video input.

Swin Transformer blocks below retain upstream copyright (Microsoft, MIT).
"""
# --------------------------------------------------------
# Swin Transformer
...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, img_size=224, num_patches=196,
    ...
    if len(pretrained_model) == 0:
        ...
        # Pretrained weights (legacy .tar); trusted source — use weights_only=False for PyTorch 2.6+.
        state_dict = torch.load(
            './pretrained_weights/swin_base_patch4_window7_224_22k.pth',
            map_location='cpu',
            weights_only=False,
        )
    else:
        ...
        if 1:
            ...
            if 'relative_position_bias_table' in key:
                ...
                if pretrained_table_size != table_size:
                    # Use reshape after permute to avoid requiring contiguous memory.
                    table = pretrained_table.permute(1, 0).reshape(
                        1, -1, pretrained_table_size, pretrained_table_size
                    )
...
@register_model
def TALL_SWIN(pretrained=True, **kwargs):
